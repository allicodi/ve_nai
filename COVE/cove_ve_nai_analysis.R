...
symp_cox_fit <- survival::coxph(survival::Surv(ftime, ftype == 1) ~ vax + 
                                  ...
                                  Black + Asian + NatAmer + PacIsl + Multiracial + RaceOther + RaceNotreported + RaceUnknown, data = cove_data)
...
msm_fit <- fit_ps_msm(
  object = ve_fit,
  which_ps_numerator = c("converted", "helpedplus"),
  which_ps_denominator = "naturally_symptomatic",
  data = cove_data,
  t0 = 175,
  msm_covariates = "Age",
)

age_range <- seq(min(cove_data$Age), max(cove_data$Age), length = 1000)
msm_pred <- predict(msm_fit$msm_fit, newdata = data.frame(Age = age_range), type = "response")
plot(msm_pred ~ age_range)

msm_fit_converted <- fit_ps_msm(
  object = ve_fit,
  which_ps_numerator = "converted",
  which_ps_denominator = "naturally_symptomatic",
  data = cove_data,
...
msm_pred_converted <- predict(msm_fit_converted$msm_fit, newdata = data.frame(Age = age_range), type = "response")
plot(msm_pred_converted ~ age_range)

msm_fit_helpedplus <- fit_ps_msm(
  object = ve_fit,
  which_ps_numerator = "helpedplus",
  which_ps_denominator = "naturally_symptomatic",
  data = cove_data,
  t0 = 175,
  msm_covariates = "Age",
...
msm_pred_helpedplus <- predict(msm_fit_helpedplus$msm_fit, newdata = data.frame(Age = age_range), type = "response")
plot(msm_pred_helpedplus ~ age_range)

# ---- 

# min is 29
# results <- ve_from_fits(
#   symp_cox_fit = symp_cox_fit, 
#   asymp_cox_fit = asymp_cox_fit, 
#   symp_lr_fit = symp_lr_fit, 
#   data = cove_data, 
...
#write_csv(ve_fit, "ve_nai/ve_nai_results.csv")

# -------

# plot at risk over time
t0 <- 1:250
at_risk <- sapply(t0, function(t) mean(cove_data$ftime > t))
